...
\begin{frame}{Analysis: AES vs Good Encryption}
	\begin{wide}
	AES perfectly aligns with the criteria for good encryption:
	
	\vspace{1em}
	\begin{itemize}
		\onslide<1->{\item \textbf{Public Design:} Open NIST competition ensured review.}
		\onslide<2->{\item \textbf{Shannon's Principles:} Exceptional confusion and diffusion.}
		\onslide<3->{\item \textbf{Key Space:} Massive keys ensure brute-force resistance.}
		\onslide<4->{\item \textbf{Attack Resistance:} Highly resilient against mathematical shortcuts.}
		\onslide<5->{\item \textbf{Efficiency:} High speed across all types of devices.}
	\end{itemize}
	\end{wide}
\end{frame}

\begin{frame}{Analysis: A Public, Well-Specified Design}
	\begin{wide}
	\begin{itemize}
		\onslide<1->{\item \textbf{The Catalyst:} In 1997, NIST sought a replacement for the aging DES cipher.}
		
		\vspace{0.6em}
		\onslide<2->{\item \textbf{The Crucible:} Instead of a secret design, NIST launched a 5-year open global competition with 15 candidates.}
		
		\vspace{0.6em}
		\onslide<3->{\item \textbf{The Victory:} In 2000, \textit{Rijndael} won, defeating submissions from industry giants like IBM and RSA.}
	\end{itemize}
	\end{wide}
\end{frame}
...
\begin{frame}{Global Trust Through Transparency}
	\begin{wide}
	\begin{itemize}
		\onslide<1->{\item \textbf{Official Standard:} Published as FIPS 197, exposing its math for worldwide scrutiny.}
		
		\vspace{0.4em}
		\onslide<2->{\item \textbf{Ultimate Endorsement:} First openly available cipher approved by the NSA for `Top Secret` data.}
	\end{itemize}
	
	\vspace{0.5em}
	\onslide<3->{
	\begin{block}{\textbf{The Takeaway: Kerckhoffs's Principle}}
		A cryptosystem must be secure even if everything about it (except the key) is public knowledge. AES earns trust not by hiding its mechanics, but by mathematically surviving decades of peer review.
	\end{block}
	}
	\end{wide}
\end{frame}
...
\begin{frame}{Shannon's Principle of Confusion}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-1.2em}
			\begin{itemize}
				\onslide<1->{\item AES achieves confusion primarily through the SubBytes operation.}
				
				\vspace{0.1em}
				\onslide<2->{\item It utilizes a highly non-linear S-Box based on the multiplicative inverse in the Galois Field GF($2^8$).}
			\end{itemize}
			
			\vspace{0.1em}
			\onslide<3->{
			\begin{block}{\textbf{The Result}}
				The relationship between the ciphertext and the symmetric key is made incredibly complex, thwarting mathematical pattern recognition.
			\end{block}
			}
		...
		\column{0.32\textwidth}
			...
			\begin{tikzpicture}[scale=0.75, transform shape]
				\node[squarenode, fill=awesomeSlateBlue!30, align=center] (k) at (0, 2) {Key};
				\node[squarenode, fill=awesomeSlateBlue!30, align=center] (c) at (0, -2) {Ciphertext};
				
				\draw[thick, awesomeCharcoal, dashed] (k) -- node[right] {\small No clear link} (c);
				\node[font=\Huge, text=buetAccent] at (0, 0) {?};
			\end{tikzpicture}
			}
	\end{columns}
	\end{wide}
...
\begin{frame}{Shannon's Principle of Diffusion}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-1.2em}
			\begin{itemize}
				\onslide<1->{\item AES achieves diffusion via the ShiftRows and MixColumns operations.}
				
				\vspace{0.1em}
				\onslide<2->{\item These steps ensure that every individual bit of the plaintext heavily influences multiple bits of the ciphertext.}
			\end{itemize}
			
			\vspace{0.1em}
			\onslide<3->{
			\begin{alertblock}{\textbf{The Avalanche Effect}}
				Statistically, flipping just a single bit in the 128-bit plaintext block alters approximately 50\% of the resulting ciphertext bits.
			\end{alertblock}
			}
		...
		\column{0.32\textwidth}
			...
			\begin{tikzpicture}[scale=0.70, transform shape]
				\node[circle, draw=buetAccent, fill=buetAccent!20, minimum size=1.2cm, inner sep=1pt, align=center] (in) at (0, 2) {1 bit\\flip};
				\node[rectangle, rounded corners, fill=awesomeSlateBlue!80, text=white, minimum width=2.5cm, minimum height=0.7cm, align=center] (aesop) at (0, 0.6) {AES Rounds};
				\node[rectangle, draw=awesomeForestGreen, fill=awesomeForestGreen!20, minimum width=3.5cm, minimum height=0.7cm, align=center] (out) at (0, -0.8) {Massive Output\\Changes};
				
				\draw[arrow] (in) -- (aesop);
				\draw[arrow] (aesop) -- (out);
			\end{tikzpicture}
			}
	\end{columns}
	\end{wide}
...
\begin{frame}{A Massive Key Space}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
			\begin{itemize}
				\onslide<1->{\item AES mandates key lengths of 128, 192, or 256 bits, exponentially increasing the computational complexity of exhaustive search.}
				
				\vspace{0.8em}
				\onslide<2->{\item A standard 128-bit key yields $2^{128}$ possible combinations (over 340 undecillion).}
				
				\vspace{0.8em}
				\onslide<3->{\item A robust Key Expansion algorithm guarantees that every derived round key is cryptographically secure and unique.}
			\end{itemize}
...
\begin{frame}{Resistance to Cryptanalysis}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
			\begin{itemize}
				\onslide<1->{\item AES was explicitly designed to mathematically neutralize established cryptanalytic techniques.}
				
				\vspace{0.8em}
				\onslide<2->{\item The algebraic complexity of the S-Box provides proven immunity against Differential and Linear Cryptanalysis.}
				
				\vspace{0.8em}
				\onslide<3->{\item The rigorous key schedule mitigates vulnerabilities against Related-Key Attacks.}
			\end{itemize}
		...
		\column{0.32\textwidth}
			...
			\begin{tikzpicture}[scale=0.85, transform shape]
				...
				\draw[thick, buetAccent] (0, 0) -- (-0.5, 0.5); % bounce off
				\draw[thick, buetAccent] (0, 0) -- (0.5, 0.5); % bounce off
			\end{tikzpicture}
			}
	\end{columns}
...
\begin{frame}{Efficiency in Software \& Hardware}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
			\begin{itemize}
				\onslide<1->{\item AES avoids computationally heavy math, relying exclusively on fast operations like XOR and bit shifts.}
				
				\vspace{0.8em}
				\onslide<2->{\item It features widespread hardware acceleration; modern CPUs execute it directly at the silicon level via instruction sets like AES-NI.}
				
				\vspace{0.8em}
				\onslide<3->{\item \textbf{The Result:} Highly scalable performance, running seamlessly on everything from constrained IoT devices to enterprise servers.}
			\end{itemize}
...
\begin{frame}{The Global Standard}
	...
	\vspace{-0.5em}
	\begin{itemize}
		\onslide<1->{\item \textbf{Ubiquity:} AES is the trusted standard securing daily digital life, from local Wi-Fi networks to international banking.}
		
		\vspace{1em}
		\onslide<2->{\item \textbf{Cryptographic Elegance:} It flawlessly implements Shannon's principles of Confusion and Diffusion through simple but highly effective byte math.}
		
		\vspace{1em}
		\onslide<3->{\item \textbf{Future-Proof:} The AES-256 variant provides a robust security margin, widely considered safe even against future quantum computing threats.}
	\end{itemize}
	\end{wide}
\end{frame}
