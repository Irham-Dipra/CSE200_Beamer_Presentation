...
\begin{frame}{\hspace{1.5em}AES vs Good Encryption}
	...
	\vspace{1.5em}
	\uncover<1->{\textbf{Public Design:} Open NIST competition ensured review.}
	
	\vspace{0.4em}
	\uncover<2->{\textbf{Shannon's Principles:} Exceptional confusion and diffusion.}
	
	\vspace{0.4em}
	\uncover<3->{\textbf{Key Space:} Massive keys ensure brute-force resistance.}
	
	\vspace{0.4em}
	\uncover<4->{\textbf{Attack Resistance:} Highly resilient against mathematical shortcuts.}
	
	\vspace{0.4em}
	\uncover<5->{\textbf{Efficiency:} High speed across all types of devices.}
	\end{wide}
\end{frame}
...
\begin{frame}{AES: A Public, Well-Specified Design}
	\begin{wide}
		...
		\vspace{0.4em}
		\uncover<2->{\textbf{The Crucible:} Instead of a secret design, NIST launched a 5-year open global competition with 15 candidates.}
		
		\vspace{0.4em}
		\uncover<3->{\textbf{The Victory:} In 2000, \textit{Rijndael} won, defeating submissions from industry giants like IBM and RSA.}
	\end{wide}
\end{frame}
...
\begin{frame}{Global Trust Through Transparency}
	\begin{wide}
		\vspace{1.5em}
		\uncover<1->{\textbf{Proven Standard:} Published openly as FIPS 197 and later approved by the NSA for `Top Secret` data, proving its mathematical resilience.}
...
\begin{frame}{Shannon's Principle of Confusion}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
				\onslide<1->{AES achieves confusion primarily through the SubBytes operation.}\\[0.4em]
				
				\onslide<2->{It utilizes a highly non-linear S-Box based on the multiplicative inverse in the Galois Field GF($2^8$).}\\[1.5em]
...
\begin{frame}{Shannon's Principle of Diffusion}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
				\onslide<1->{AES achieves diffusion via the ShiftRows and MixColumns operations.}\\[0.4em]
				
				\onslide<2->{These steps ensure that every individual bit of the plaintext heavily influences multiple bits of the ciphertext.}\\[1.5em]
...
\begin{frame}{A Massive Key Space}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
				\onslide<1->{AES mandates key lengths of 128, 192, or 256 bits, exponentially increasing the computational complexity of exhaustive search.}\\[0.8em]
				
				\onslide<2->{A standard 128-bit key yields $2^{128}$ possible combinations (over 340 undecillion).}\\[0.8em]
				
				\onslide<3->{A robust Key Expansion algorithm guarantees that every derived round key is cryptographically secure and unique.}
...
\begin{frame}{Resistance to Cryptanalysis}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
				\onslide<1->{AES was explicitly designed to mathematically neutralize established cryptanalytic techniques.}\\[0.8em]
				
				\onslide<2->{The algebraic complexity of the S-Box provides proven immunity against Differential and Linear Cryptanalysis.}\\[0.8em]
				
				\onslide<3->{The rigorous key schedule mitigates vulnerabilities against Related-Key Attacks.}
...
\begin{frame}{Efficiency in Software \& Hardware}
	...
	\begin{columns}[T,onlytextwidth]
		\column{0.68\textwidth}
			\vspace{-0.5em}
				\onslide<1->{AES avoids computationally heavy math, relying exclusively on fast operations like XOR and bit shifts.}\\[0.8em]
				
				\onslide<2->{It features widespread hardware acceleration; modern CPUs execute it directly at the silicon level via instruction sets like AES-NI.}\\[0.8em]
				
				\onslide<3->{\textbf{The Result:} Highly scalable performance, running seamlessly on everything from constrained IoT devices to enterprise servers.}
